		...
		<div class="row">
			<div class="col-md-12">
				<form id="bbs_form" action="${contextPath}/bbs/board/save.do" method="post" enctype="multipart/form-data" onsubmit="return fn_onsubmit(this);">
					...
					<input type="hidden" name="bbs" value="${bbsInfo.bbs_uid}"/>
					<c:if test="${contentInfo != null }">
						<input type="hidden" name="seq" value="${contentInfo.content_seq}"/>
					</c:if>
								...
								<td>
									<div style="width:100%;">
										<div>
											...
											<div class="col-lg-12">
                                          		<input type="text" id="title" name="title" class="form-control" value="<c:if test='${contentInfo != null}'>${contentInfo.title}</c:if>" required/>
                                     		</div>
										</div>
										<div>
...
<script type="text/javascript">
	function fn_list() {
		location.href='${contextPath}/bbs/board.do?bbs=${bbsInfo.bbs_uid}';	
	}
...
//             accept: 'jpg|png|gif', //허용할 확장자(지정하지 않으면 모든 확장자 허용)
	...
	$(document).ready(function() {
		...
		$("#bbs-content").data("kendoEditor").value('${contentInfo.content}');
// 		$("#bbs-content").data("kendoEditor").value('<p>bbs-content</p><p>게시판 내용</p><p><strong>굵게</strong></p><p><span style="text-decoration:underline;">밑줄</span></p><p><em>기울임""</em></p>');
