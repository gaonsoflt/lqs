            ...
            $(document).ready(function () {
				var G_CATGR;//마스터에서 선택한 분류코드 값
					...
					dataSourceMaster = new kendo.data.DataSource({
						transport: {
							read:  {
								url: crudServiceBaseUrl + "/readParent.do",
		            			dataType: "jsonp",
		            			complete: function(e){ 
					...
					dataSourceDetail = new kendo.data.DataSource({
						transport: {
							read:  { url: crudServiceBaseUrl + "/readChild.do", dataType: "jsonp",
		            			complete: function(e){ 
		            				console.log("read:complete");
		            			}
							},
...
/* 							if (!confirm("변경된 데이타를 저장하시겠습니까?")) {  
					...
					$("#gridDetail").kendoGrid({
						...
						columns: [
							{ field: "cd_nm", title: "코드명", width: 200, attributes: {style: "text-align: center;"} },
							{ field: "cd_id", title: "코드ID", width: 250, attributes: {style: "text-align: center;"} },
// 							{ field: "cd", title: "코드", width: 150, attributes: {style: "text-align: center;"} },
						...
						save: function(e) {//저장전 이벤트
							console.log("save");
							if(G_CATGR == null || typeof G_CATGR == 'undefinded') {
								e.preventDefault();
